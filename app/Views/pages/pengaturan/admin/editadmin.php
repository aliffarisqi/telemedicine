...
<div class="container bg-white mt-4 p-4 mb-4 shadow-sm">
    <div class="row">
        <div class="col bg-light m-2 p-4">
            <form action="<?= base_url('pengaturan/admin/daftaradmin/updateproses'); ?>" method="POST" enctype="multipart/form-data">
                <?= csrf_field(); ?>
                <input type="hidden" value="<?= $dataadmin['id_admin']; ?>" name="id">
                <input type="hidden" value="<?= $dataadmin['password_admin']; ?>" name="passwordlama">
                <div class="form-group">
                    <label for="username" class="text-dark">
                        <h6>Username</h6>
                    </label>
                    <input type="text" class="form-control <?= ($validation->hasError('username')) ? 'is-invalid' : ''; ?>" id="username" value="<?= (old('username')) ? old('username') : $dataadmin['username_admin']; ?>" name="username" placeholder="nama lengkap ...">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control <?= ($validation->hasError('email')) ? 'is-invalid' : ''; ?>" id="email" value="<?= (old('email')) ? old('email') : $dataadmin['email_admin']; ?>" name="email" placeholder="email ...">
                    <div class="invalid-feedback">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control " id="puskesmas" name="puskesmas">
                        <?php foreach ($datapuskesmas as $dp) : ?>
                            <?php if ($dp['id_puskesmas'] == $dataadmin['id_puskesmas']) { ?>
                                <option value="<?= $dp['id_puskesmas']; ?>" selected><?= $dp['nama_puskesmas']; ?></option>
                            <?php } else { ?>
                                <option value="<?= $dp['id_puskesmas']; ?>"><?= $dp['nama_puskesmas']; ?></option>
                            <?php } ?>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select class="form-control " id="status" name="status">
                        <?php if ($dataadmin['status'] == "Super Admin") { ?>
                ...
                <hr class="mt-4 mb-4">
                <button type="submit" class="btn btn-warna-orange ml-2 float-right">save</button>
